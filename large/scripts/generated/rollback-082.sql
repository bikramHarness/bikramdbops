...
declare
    v_exists number;
begin
    select count(*) into v_exists from user_tables where table_name = 'TMP_STAGE_82';
    if v_exists > 0 then
        execute immediate 'drop table TMP_STAGE_82 purge';
    end if;
end;
/

begin
    for idx in (select index_name from user_indexes where table_name = 'TMP_STAGE_82') loop
        execute immediate 'drop index ' || idx.index_name;
    end loop;
end;
/

prompt Completed rollback for changeset 82
