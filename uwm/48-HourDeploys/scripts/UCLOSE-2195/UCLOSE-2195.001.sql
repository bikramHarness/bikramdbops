...
/****** Object:  StoredProcedure [LNDATA].[usp_Select_Loan]    Script Date: 3/20/2025 8:39:49 AM ******/
SET ANSI_NULLS ON
GO
...
--	FAng						03/20/2025			UCLOSE-2195 - Tech Debt | LNDATA Stored Procedure
********************************************************************************************/
...
BEGIN
    ...
    JOIN      dbo.setups_loanstatus          AS ls WITH (NOLOCK)
      ON lm.statusid               = ls.statusid
    LEFT JOIN dbo.loan_automatedunderwriting AS law WITH (NOLOCK)
           ON law.lenderdatabaseid = lm.lenderdatabaseid
          AND law.loanrecordid     = lm.loanrecordid
    LEFT JOIN dbo.custom_loanmain            AS clm WITH (NOLOCK)
           ON clm.lenderdatabaseid = lm.lenderdatabaseid
          AND clm.loanrecordid     = lm.loanrecordid
    LEFT JOIN dbo.loan_processing            AS lp WITH (NOLOCK)
    ...
    LEFT JOIN dbo.USFS_LoanClosingLLC            AS lcl WITH (NOLOCK)
    ...
    LEFT JOIN dbo.property_main              AS pm WITH (NOLOCK)
